...
def dict_tempo_create(current_year, even_year, year_letter):
    ...
    dict_tempo = {}
    # Ce dictionnaire sera utilisé pour le tableau des fêtes mobiles en page 3 de l'ordo.
    dict_tempo["fetes_mobiles"] = {}
    ...
    for i in range(2):
        new_dim_adv_date = sabb_1_adv_date + \
            datetime.timedelta(days=1 + (i * 7))
        if i == 0:
            dict_tempo["fetes_mobiles"]["dim_1_adv"] = str(
                new_dim_adv_date.day) + f_transf_month_genitive(new_dim_adv_date.month)
        new_dim_adv = dict_tempo[new_dim_adv_date] = {}
        new_dim_adv["force"] = 120
        new_dim_adv["I_vesp"] = "\\item I Vesperæ dominicæ sequentis."
        new_dim_adv["hebdo_psalt"] = "- Hebdomada " + hebdo_psalterii[(
            (new_dim_adv_date - datetime.date(2011, 11, 27)).days // 7) % 2] + " psalterii -"
        new_dim_adv["num_day"] = "\\textbf{" + \
            str(new_dim_adv_date.day) + "} &"
        ...
        lectiones_body = "\n\\item[Dom. " + year_letter + \
            "] " + current_lectiones["dim"][year_letter]
        ...
        for j in range(6):
            if i == 0 and j == 0:
                ...
                lectiones_body += "\n\\item[" + f_transf_weekday(
                    j) + "] " + current_lectiones[j][year_letter_a_or_bc]
            else:
                lectiones_body += "\n\\item[" + \
                    f_transf_weekday(j) + "] " + current_lectiones[j]
            new_day_adv_date = new_dim_adv_date + \
                datetime.timedelta(days=j + 1)
            new_day_adv = dict_tempo[new_day_adv_date] = {}
            new_day_adv["force"] = 10
            new_day_adv["num_day"] = "\\textbf{" + str(
                new_day_adv_date.day) + "} & " + f_transf_weekday(new_day_adv_date.weekday())
    ...
    dim_3_adv["hebdo_psalt"] = "- Hebdomada " + hebdo_psalterii[(
        (dim_3_adv_date - datetime.date(2011, 11, 27)).days // 7) % 2] + " psalterii -"
    ...
    if dim_3_adv_date.day != 17:
        ...
        for i in range(dim_3_adv_date.day - 10):
            if (17 + i) != 21:
                lectiones_body += "\n\\item[" + \
                    str(17 + i) + " dec.] " + current_lectiones[17 + i]
            else:
                lectiones_body += "\n\\item[21 dec.] " + (
                    current_lectiones[21]["C"] if year_letter == "C" else current_lectiones[21]["AB"])
    else:
        current_lectiones = lectiones["ultimae_feriae"]
        for i in range(6):
            lectiones_body += "\n\\item[" + str(18 + i) + " dec.] " + (current_lectiones[18 + i] if (
                18 + i) != 21 else (current_lectiones[21]["C"] if year_letter == "C" else current_lectiones[21]["AB"]))
    ...
    lect_vigiles = {2: "\\item Ad Vigilias: lectiones SO in supplemento 5.",
                    3: "\\item Ad Vigilias: lectiones SO in supplemento 6.", 4: "\\item Ad Vigilias: lectiones SO in supplemento 8."}
    ...
    for i in range(6):
        ...
        if new_day_date.day < 17:
            new_day["force"] = 10
            lect_vigiles_txt = lect_vigiles[new_day_date.weekday(
            )] if new_day_date.weekday() in lect_vigiles else ""
        else:
            new_day["force"] = 50
            if new_day_date.day == 17:
                new_day["generalities"] = generalities
            ...
            if new_day_date.weekday() == 3 or new_day_date.weekday() == 4:
                div_longs_psaumes = "\\item Ad Vesperas: divisiones psalmorum considerantur ut integri psalmi, ita ut eis antiphonæ propriæ sint attribuendæ."
        ...
        new_day["body"] = lect_vigiles_txt + ad_omnes_horas_txt + office_quatre_temps + \
            ant_bened_txt + messe_basse_quatre_temps + div_longs_psaumes
    ...
    dim_4_adv["hebdo_psalt"] = "- Hebdomada " + hebdo_psalterii[(
        (dim_4_adv_date - datetime.date(2011, 11, 27)).days // 7) % 2] + " psalterii -"
    ...
    if dim_4_adv_date.day == 21:
        ant_bened_special = "\n\\item Ad Benedictus: ø \\textit{Nolite timere (AM 219)}."
    elif dim_4_adv_date.day == 23:
        ant_bened_special = "\n\\item Ad Benedictus: ø \\textit{Ecce completa sunt (AM 220)}."
    else:
        ant_bened_special = ""
    ...
    for i in range(24 - dim_4_adv_date.day):
        if (dim_4_adv_date.day + i + 1) != 21:
            lectiones_body += "\n\\item[" + str(dim_4_adv_date.day + i + 1) + \
                " dec.] " + current_lectiones[dim_4_adv_date.day + i + 1]
        else:
            lectiones_body += "\n\\item[21 dec.] " + (
                current_lectiones[21]["C"] if year_letter == "C" else current_lectiones[21]["AB"])
    ...
    for i in range(6):
        ...
        if new_day_date.day < 25:
            ...
            if new_day_date.day == 21:
                ...
                ant_bened_txt = ant_bened_adv_4[new_day_date.weekday(
                )] if new_day_date.weekday() != 5 else ""
            if new_day_date.weekday() == 3 or new_day_date.weekday() == 4:
                div_longs_psaumes = "\\item Ad Vesperas: divisiones psalmorum considerantur ut integri psalmi, ita ut eis antiphonæ propriæ sint attribuendæ."

            # On concatène le tout:
            new_day["body"] = ad_omnes_horas_txt + \
                ant_bened_txt + div_longs_psaumes

    # Vigile de Noël:
    vigile_noel_date = noel_date - datetime.timedelta(days=1)
    # Car si 24/12 = dimanche, alors c'est le 4e dim. de l'Avent.
    if vigile_noel_date.weekday() != 6:
        vigile_noel = dict_tempo[vigile_noel_date] = {}
        vigile_noel["force"] = 50
        vigile_noel["num_day"] = "\\textbf{" + str(
            vigile_noel_date.day) + "} & " + f_transf_weekday(vigile_noel_date.weekday())
    ...
    noel["num_day"] = "\\textbf{" + str(noel_date.day) + "} & " + \
        f_transf_weekday(noel_date.weekday())
    noel["header"] = ("Dominica - " if noel_date.weekday() == 6 else " - ") + \
    ...
    if noel_date.weekday() == 0 or noel_date.weekday() == 1 or noel_date.weekday() == 6:
        ...
        lectiones_body += "\n\\item[30 dec.] " + current_lectiones[30] if noel_date.weekday(
        ) != 1 and noel_date.weekday() != 6 else ""
        lectiones_body += "\n\\item[31 dec.] " + \
            current_lectiones[31] if noel_date.weekday() != 0 else ""
    noel["lectiones_body"] = lectiones_body

    # Sainte Famille:
    if noel_weekday == 6:
        ste_famille_date = noel_date + datetime.timedelta(days=5)
    else:
        ste_famille_date = noel_date + \
            datetime.timedelta(days=6 - noel_weekday)
    dict_tempo["fetes_mobiles"]["ste_famille"] = str(
        ste_famille_date.day) + f_transf_month_genitive(ste_famille_date.month)
    ...
    ste_famille["I_vesp"] = "\\item I Vesperæ festi sequentis (AM 265)." if ste_famille_date.weekday(
    ) == 6 else ""
    ...
    if ste_famille_date.day != 30 and ste_famille_date.day != 31:
        ...
        for i in range(3):
            if (29 + i) != ste_famille_date.day:
                lectiones_body += "\n\\item[" + \
                    str(29 + i) + " dec.] " + current_lectiones[29 + i]
    ...
    for i in range(7):
        ...
        if new_day_date != ste_famille_date:
            ...
            benedicamus_domino = "\\item Ad Laudes et Vesperas: ß \\textit{Benedicamus Domino} III." if new_day_date.weekday(
    ...
    dim_2_ap_noel["hebdo_psalt"] = "- Hebdomada " + hebdo_psalterii[(
        (dim_2_ap_noel_date - datetime.date(2011, 11, 27)).days // 7) % 2] + " psalterii -"
    dim_2_ap_noel["num_day"] = "\\textbf{" + \
        str(dim_2_ap_noel_date.day) + "} &"
    ...
    lect_propres = " Lectiones propriæ: " + lectures_mc + \
        ";" if dim_2_ap_noel_date.day == 5 else ""
    dim_2_ap_noel["body"] = "\\item Ad Laudes et Vesperas: antiphonæ et psalmi de psalterio, reliqua ut in dominica infra octavam Nativitatis (AM 266).\n\\item Ad Horas minores: antiphonæ de Circumcisione, reliqua ut in dominica infra octavam Nativitatis (AM 266)." + \
    ...
    if dim_2_ap_noel_date.day != 1 and dim_2_ap_noel_date.day != 5:
        ...
        for i in range(6 - dim_2_ap_noel_date.day - 1):
            lectiones_body += "\n\\item[" + str(dim_2_ap_noel_date.day + i + 1) + \
                " ian.] " + current_lectiones[dim_2_ap_noel_date.day + i + 1]
    ...
    bapteme["hebdo_psalt"] = "- Hebdomada " + hebdo_psalterii[(
        (bapteme_date - datetime.date(2011, 11, 27)).days // 7) % 2] + " psalterii -"
    hebdo_psalt_vigiles = "; psalmi anni {}".format(
        hebdo_psalterii_inv[even_year])
    bapteme["num_day"] = "\\textbf{" + str(bapteme_date.day) + "} &"
    bapteme["symbols"] = "".join(
        f_symbols(bapteme_date)) + (" " if f_symbols(bapteme_date) != "" else "")
    ...
    for i in range((bapteme_date - noel_date).days + 6):
        ...
        if new_day_date != dim_2_ap_noel_date and new_day_date != bapteme_date:
            ...
            new_day["num_day"] = "\\textbf{" + str(
                new_day_date.day) + "} & " + f_transf_weekday(new_day_date.weekday())
            new_day["symbols"] = "".join(f_symbols(new_day_date))
            new_day["header"] = " - de ea - \\textit{Alb}." if new_day_date.weekday(
            ) != 5 else " - de eo - \\textit{Alb}."
    ...
    dict_tempo["fetes_mobiles"]["cendres"] = str(
    ...
    for i in range(4):
        lectiones_body += "\n\\item[" + \
            f_transf_weekday(i + 2) + "] " + current_lectiones[i]
    ...
    for i in range(4):
        new_dim_careme_date = cendres_date + \
            datetime.timedelta(days=4 + (i * 7))
        ...
        new_dim_careme["hebdo_psalt"] = "- Hebdomada " + hebdo_psalterii[(
            (new_dim_careme_date - datetime.date(2011, 11, 27)).days // 7) % 2] + " psalterii -"
        new_dim_careme["num_day"] = "\\textbf{" + \
            str(new_dim_careme_date.day) + "} &"
        ...
        new_dim_careme["II_vesp"] = "\n\\item Vesperæ dominicæ." if new_dim_careme_date.month == 3 and (
            new_dim_careme_date.day == 19 or new_dim_careme_date.day == 25) else ""
        new_dim_careme["preface_feries"] = "\n\\item In feriis: præfatio " + \
            f_roman_numbers(i + 1) + " de Quadragesima, nisi aliter notetur."
        current_lectiones = lectiones["hebdo_" + str(i + 1) + "_quadr"]
        new_dim_careme["lectiones_header"] = current_lectiones["header"]
        lect_first_line_A = "\n\\item[Dom. A] " + current_lectiones["dim"]["A"] if (
            i == 2 or i == 3) and year_letter != "A" else ""
        lectiones_body = lect_first_line_A + \
            "\n\\item[Dom. " + year_letter + "] " + \
            current_lectiones["dim"][year_letter]
        new_dim_careme["II_vesp"] = "\\item Vesperæ dominicæ." if new_dim_careme_date.month == 3 and (
            new_dim_careme_date.day == 18 or new_dim_careme_date.day == 19) else ""

        for j in range(6):
            lectiones_body += "\n\\item[" + \
                f_transf_weekday(j) + "] " + current_lectiones[j]
            new_day_careme_date = new_dim_careme_date + \
                datetime.timedelta(days=j + 1)
            new_day_careme = dict_tempo[new_day_careme_date] = {}
            new_day_careme["force"] = 10
            new_day_careme["num_day"] = "\\textbf{" + str(
                new_day_careme_date.day) + "} &" + f_transf_weekday(new_day_careme_date.weekday())
            new_day_careme["symbols"] = "".join(
                f_symbols(new_day_careme_date, "Quadr"))
    ...
    lect_first_line_A = "\n\\item[Dom. A] " + \
        current_lectiones["dim"]["A"] if year_letter != "A" else ""
    lectiones_body = lect_first_line_A + \
        "\n\\item[Dom. " + year_letter + "] " + \
        current_lectiones["dim"][year_letter]
    passion["II_vesp"] = "\\item Vesperæ dominicæ." if passion_date.month == 3 and (
        passion_date.day == 18 or passion_date.day == 19) else ""
    ...
    dict_tempo["fetes_mobiles"]["rameaux"] = str(
        rameaux_date.day) + f_transf_month_genitive(rameaux_date.month)
    ...
    rameaux["hebdo_psalt"] = "- Hebdomada " + hebdo_psalterii[(
        (rameaux_date - datetime.date(2011, 11, 27)).days // 7) % 2] + " psalterii -"
    ...
    lectiones_body = "\n\\item[Dom. " + year_letter + "] " + \
        "\\textit{Ante processionem:} " + \
        current_lectiones["ante_proc"][year_letter]
    lectiones_body += "\n\\item[Dom. " + year_letter + \
        "] " + current_lectiones["dim"][year_letter]
    for i in range(3):
        lectiones_body += "\n\\item[" + \
    ...
    rameaux["II_vesp"] = "\\item Vesperæ dominicæ." if rameaux_date.month == 3 and (
        rameaux_date.day == 18 or rameaux_date.day == 19) else ""
    ...
    vendredi_saint["num_day"] = "\\textbf{" + \
        str(vendredi_saint_date.day) + "} &"
    ...
    dict_tempo["fetes_mobiles"]["paques"] = str(
        dim_paques_date.day) + f_transf_month_genitive(dim_paques_date.month)
    ...
    for i in range(6):
        ...
        fer_ap_paques["num_day"] = "\\textbf{" + str(
            fer_ap_paques_date.day) + "} &" + f_transf_weekday(fer_ap_paques_date.weekday())
    ...
    for i in range(5):
        ...
        dim_ap_paques["hebdo_psalt"] = "- Hebdomada " + hebdo_psalterii[(
            (dim_ap_paques_date - datetime.date(2011, 11, 27)).days // 7) % 2] + " psalterii -"
        dim_ap_paques["num_day"] = "\\textbf{" + \
            str(dim_ap_paques_date.day) + "} &"
        dim_ap_paques["symbols"] = "".join(f_symbols(dim_ap_paques_date))
        dim_ap_paques["header"] = (" \\textbf{\\textsc{Dominica " + f_roman_numbers(i + 2) + " Paschæ}} (seu Dominica " + f_roman_numbers(
        ...
        for j in range(6):
            if (i == 2) and (j == 0):
                lectiones_body += "\n\\item[" + f_transf_weekday(j) + "] " + (
                    current_lectiones[j]["A"] if year_letter == "A" else current_lectiones[j]["BC"])
            else:
                lectiones_body += "\n\\item[" + f_transf_weekday(
                    j) + "] " + current_lectiones[j] if j in current_lectiones else ""
            fer_tp_date = dim_ap_paques_date + datetime.timedelta(days=j + 1)
            fer_tp = dict_tempo[fer_tp_date] = {}
            fer_tp["force"] = 10
            fer_tp["num_day"] = "\\textbf{" + str(
                fer_tp_date.day) + "} &" + f_transf_weekday(fer_tp_date.weekday())
        ...
        if i == 4:
            ...
            lectiones_ascension = "\n\\ApplyLectHeader{" + \
                current_lectiones["header"] + "}"
            lectiones_ascension += "\n\\ApplyLectBody{\n\\item[Anno " + year_letter + \
                "] " + current_lectiones["asc"][year_letter] + "\\smallskip"
            for i in range(2):
                lectiones_ascension += "\n\\item[" + \
                    f_transf_weekday(i + 4) + "] " + current_lectiones[i]
            dim_ap_paques["lectiones_body"] += "}\n" + lectiones_ascension

    # Ascension:
    ascension_date = paques_date + datetime.timedelta(days=39)
    dict_tempo["fetes_mobiles"]["ascension"] = str(
    ...
    dim_7_paques["I_vesp"] = "\n\\item I Vesperæ dominicæ sequentis."
    dim_7_paques["hebdo_psalt"] = "- Hebdomada " + hebdo_psalterii[(
        (dim_7_paques_date - datetime.date(2011, 11, 27)).days // 7) % 2] + " psalterii -"
    ...
    dict_tempo["fetes_mobiles"]["pentecote"] = str(
    ...
    dict_tempo["fetes_mobiles"]["fete_dieu"] = str(
        fete_dieu_date.day) + f_transf_month_genitive(fete_dieu_date.month)
    fete_dieu = dict_tempo[fete_dieu_date] = {}
    fete_dieu["force"] = 115  # Ainsi cette solennité l'emporte sur saint J.B.
    fete_dieu["I_vesp"] = "\n\\item I Vesperæ sollemnitatis sequentis."
    fete_dieu["num_day"] = "\\textbf{" + \
        str(fete_dieu_date.day) + "} &" + " Feria V"
    fete_dieu["header"] = " - ¬ \\textbf{\\MakeUppercase{Ss.mi Corporis et Sanguinis Christi} - sollemnitas maior} - \\textit{Alb}."
    lectiones_fd = {"A": "Deut \\textbf{8}, 2-3.14b-16a / 1 Co \\textbf{10}, 16-17 / Io \\textbf{6}, 51-59",
    ...
    dict_tempo["fetes_mobiles"]["sacre_coeur"] = str(
        sacre_coeur_date.day) + f_transf_month_genitive(sacre_coeur_date.month)
    sacre_coeur = dict_tempo[sacre_coeur_date] = {}
    # Ainsi cette solennité l'emporte sur saint J.B. et SS. Pierre et Paul.
    sacre_coeur["force"] = 115
    sacre_coeur["I_vesp"] = "\n\\item I Vesperæ sollemnitatis sequentis."
    sacre_coeur["num_day"] = "\\textbf{" + \
        str(sacre_coeur_date.day) + "} &" + " Feria VI"
    ...
    # Pour être plus fort qu'une mémoire majeure du Sancto.
    coeur_imm_marie["force"] = 50
    coeur_imm_marie["num_day"] = "\\textbf{" + \
        str(coeur_imm_marie_date.day) + "} &" + " Sabbato"
    coeur_imm_marie["symbols"] = " µ" if sacre_coeur_date.day < 8 else "" + \
    ...
    for i in range(nb_dim_per_annum_ap_bapteme):
        ...
        if i != 0:
            ...
            comment_septua = "\\item In Officio: omnia dicuntur sicut in psalterio, præter antiphonas ad Benedictus et Magnificat." + \
                ("\n\\item {\\itshape In ML (Viol.): non dicitur \\emph{Gloria} in Missis de tempore ab hac Dominica usque ad feriam IV Hebdomadæ Sanctæ inclusive.}" if new_dim_date ==
                 cendres_date - datetime.timedelta(days=17) else "\n\\item {\\itshape In ML: Viol.}")
            ...
            new_dim["force"] = 80
            new_dim["I_vesp"] = "\n\\item I Vesperæ " + \
                ("festi" if new_dim_date.day == 2 and new_dim_date.month ==
                 2 else "dominicæ") + " sequentis" + forme_extra_I_vesp + "."
            ...
            new_dim["header"] = " \\textbf{\\textsc{Dominica " + num_dim_per_annum + \
            ...
            new_dim["body"] = comment_septua + "\n\\item In MC: præfatio " + num_pref_dim + " de dominicis." + \
                ("\n\\item Ad benedictionem Sanctissimi Sacramenti, post canticum expositionis, cantatur \\textit{Ubi Caritas} in Besnier 275 (hebdomada pro christianorum unitate)." if new_dim_date.day <
                 25 and new_dim_date.day >= 18 and new_dim_date.month == 1 else "")
            new_dim["II_vesp"] = "}\n\\ApplyGenerSubTitle{In Officio}\n\\ApplyGenerList{\n\\item In feriis usque ad tempus quadragesimæ : antiphona ad Benedictus ut in psalterio; ad Magnificat vero antiphona propria (AM 316 et sequentibus).}\n\\ApplyGenerSubTitle{In ML}\n\\ApplyGenerList{\n\\item Post graduale dicitur tractus, præterquam in feriis quando resumitur Missa dominicæ.\n\\item In feriis Septuagesimæ, quando sacerdos celebrat paramentis violaceis, dicere debet vel Missam dominicæ præcedentis vel Missam votivam ex eis quæ celebrantur colore violaceo." if new_dim_date == cendres_date - datetime.timedelta(
                days=17) else ""
        ...
        # Lectures après la Quinquagésime : seulement lundi et mardi.
        for j in range(6 if i != (nb_dim_per_annum_ap_bapteme - 1) else 2):
            lectiones_body += "\n\\item[" + f_transf_weekday(
                j) + "] " + current_lectiones[j][even_year_num] + " / " + current_lectiones[j][2] if i != 0 else ""
            ...
            if new_day_date < cendres_date:
                ...
                new_day["body"] = "\n\\item \\textit{In ML: Viol.}" if new_day_date > cendres_date - datetime.timedelta(
                    days=17) else ""
        ...
        if i != 0:
            new_dim["lectiones_body"] = lectiones_body
    ...
    for i in range(nb_dim_ap_pentec):
        ...
        if i != 0:
            if i == 1:  # Trinité.
                dict_tempo["fetes_mobiles"]["trinite"] = str(
                    new_dim_date.day) + f_transf_month_genitive(new_dim_date.month)
                # Numéro du Dimanche per annum:
                num_dim_per_annum = f_roman_numbers(36 - nb_dim_ap_pentec)
                # Numéros de la préface commune pour les féries:
                num_pref_fer = f_roman_numbers(
                    (36 - nb_dim_ap_pentec) % 6) if ((i + 1) % 6 != 0) else "VI"
                ...
                trinite["num_day"] = "\\textbf{" + \
                    str(new_dim_date.day) + "} &"
                trinite["symbols"] = "".join(
                    f_symbols(new_dim_date)) + (" " if f_symbols(new_dim_date) != "" else "")
                ...
                trinite_lectiones = {"A": "Ex \\textbf{34}, 4b-6.8-9 / 2 Co \\textbf{13}, 11-13 / Io \\textbf{3}, 16-18",
                                     "B": "Deut \\textbf{4}, 32-34.39-40 / Rom \\textbf{8}, 14-17 / Mt \\textbf{28}, 16-20", "C": "Prov \\textbf{8}, 22-31 / Rom \\textbf{5}, 1-5 / Io \\textbf{16}, 12-15"}
                current_lectiones = lectiones["hebdo_" +
                                              str(36 - nb_dim_ap_pentec) + "_per_annum"]
                lectiones_body = "\n\\ApplyLectHeader{" + \
                    current_lectiones["header"] + "}"
                lectiones_body += "\n\\ApplyLectBody{"
                for j in range(6):
                ...
                lectiones_feries = "\n\\ApplyLectHeader{" + \
                    current_lectiones["header_feries"] + "}"
                lectiones_feries += "\n\\ApplyLectBody{"
                for j in range(6):
                    lectiones_feries += "\n\\item[" + f_transf_weekday(
                        j) + "] " + current_lectiones[j][even_year_num] + " / " + current_lectiones[j][2]
                ...
                num_dim_ap_pentec = f_roman_numbers(
                    i) + " post Pentecosten" if i < 24 else f_roman_numbers(i - nb_dim_ap_pentec + 8) + " post Epiphaniam"
                if i == 2:
                    num_dim_ap_pentec = "II post Pentecosten vel infra octavam Ss. Corporis Christi" if i < 24 else f_roman_numbers(
                        i - nb_dim_ap_pentec + 8) + " post Epiphaniam"
                elif i == 3:
                    num_dim_ap_pentec = "III post Pentecosten vel infra octavam Ss. Cordis Iesu" if i < 24 else f_roman_numbers(
                        i - nb_dim_ap_pentec + 8) + " post Epiphaniam"
                # Numéro du dimanche d'été (août à novembre):
                num_summer = " - " + f_num_summer(new_dim_date)[0] + f_num_summer(new_dim_date)[
                    1] if (new_dim_date.month >= 8) and (new_dim_date.month <= 11) else ""
                # Numéros des préfaces:
                num_pref_dim, num_pref_fer = f_num_prefaces(
                    i + 34 - nb_dim_ap_pentec)
                ...
                elif num_summer == " - I augusti":
                    body_special = "\n\\item Ad Vigilias: lectiones I et II Nocturnorum sumendæ sunt e dominica I augusti, lectiones III Nocturni e dominica post Pentecosten, et sic usque ad Adventum."
                elif num_summer == " - V octobris":
                    body_special = "\n\\item Ad Vigilias: lectiones I et II Nocturnorum in supplemento 51*." if even_year else "\n\\item Ad Vigilias: lectiones I Nocturni in supplemento 51*."
                elif num_summer == " - II novembris":
                    body_special = "\n\\item Ad Vigilias: lectiones I et II Nocturnorum sumuntur in supplemento 52*. Et sic in feriis hebdomadæ II novembris." if even_year else "\n\\item Ad Vigilias: lectiones I Nocturni sumuntur in supplemento 52*. Et sic in feriis hebdomadæ II novembris."
                else:
                    body_special = ""
                ...
                new_dim["preface_feries"] = "\n\\item In feriis: præfatio communis " + \
                    num_pref_fer + ", nisi aliter notetur."
                if num_summer == " - I novembris":
                    new_dim["preface_feries"] += "\n\\item Ad Vigilias: in feriis, lectiones SO."
                current_lectiones = lectiones["hebdo_" +
                                              str(i + 35 - nb_dim_ap_pentec) + "_per_annum"]
                new_dim["lectiones_header"] = current_lectiones["header"]
                lectiones_body = ("\n\\item[Dom. " + year_letter + "] " + current_lectiones["dim"][year_letter]) if new_dim_date != datetime.date(current_year, 6, 24) and new_dim_date != datetime.date(current_year, 6, 29) and new_dim_date != datetime.date(current_year, 7, 11) and new_dim_date != datetime.date(current_year, 8, 6) and new_dim_date != datetime.date(current_year, 8, 15) and new_dim_date != datetime.date(
                    current_year, 9, 7) and new_dim_date != datetime.date(current_year, 9, 14) and new_dim_date != datetime.date(current_year, 11, 1) and new_dim_date != datetime.date(current_year, 11, 2) and new_dim_date != datetime.date(current_year, 11, 9) else ""  # Ne pas mentionner les lectures du dimanche si ce dimanche est une solennité ou une fête du Seigneur (lectures propres).

        else:  # Si i == 0 : lectiones semaine Pentecôte.
            current_lectiones = lectiones["hebdo_" +
                                          str(35 - nb_dim_ap_pentec) + "_per_annum"]
            lectiones_body = "\n\\ApplyLectHeader{" + \
                current_lectiones["header"] + "}"
            lectiones_body += "\n\\ApplyLectBody{"
            for j in range(6):
        ...
        for j in range(6):
            if i != 0:
                if num_dim_per_annum == "XVIII" and j in [0, 1]:
                    year_letter_a_or_bc = "A" if year_letter == "A" else "BC"
                    lectiones_body += "\n\\item[" + f_transf_weekday(
                        j) + "] " + current_lectiones[j][year_letter_a_or_bc][even_year_num] + " / " + current_lectiones[j][year_letter_a_or_bc][2]
                else:
                    lectiones_body += "\n\\item[" + f_transf_weekday(
                        j) + "] " + current_lectiones[j][even_year_num] + " / " + current_lectiones[j][2]
            else:
            ...
            if new_day_date != fete_dieu_date and new_day_date != sacre_coeur_date and new_day_date != coeur_imm_marie_date:
                new_day = dict_tempo[new_day_date] = {}
                new_day["force"] = 10
                if i == nb_dim_ap_pentec - 1 and j == 0 and even_year and f_num_summer(christ_roi_date)[0] != "III":
                    # Après le Christ-Roi, si IVe sem. de Novembre et années paires : lectures de la Ve sem. de novembre.
                ...
                if new_day_date.weekday() == 5:
                    ...
                    if (i == 0 and new_day_date == new_dim_date + datetime.timedelta(days=6) and new_day_date.month == 6 and new_day_date.day > 14 and new_day_date.day < 22):
                        ...
                        new_day["body"] = f_mc_bmv(new_day_date, paques_date)
        if (i != 0) and (i != 1) and (i != nb_dim_ap_pentec - 1):
            new_dim["lectiones_body"] = lectiones_body
