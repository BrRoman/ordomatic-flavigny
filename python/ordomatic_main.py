...
dict_tempo, ordo_start, nb_days, date_paques, nb_dim_ap_pentec, christ_roi = dict_tempo_create(
    year, even_year, year_letter)
...
text_ordo += "\n\\newcommand{\\CurrentYear}{" + str(year - 1) + "}"
text_ordo += "\n\n\\begin{document}"
text_ordo += "\n\n\\thispagestyle{empty}"
text_ordo += "\n\n\\begin{center}"
...
text_ordo += "\n\\thispagestyle{empty}"
text_ordo += "\n\\vspace{1cm}\\ApplyGenerTitleLarge{Advertenda}\\par"
...
for i in range(nb_days):
    ...
    if new_day_date == date_paques - datetime.timedelta(days=44):
        dict_new_day[
    ...
    if new_day_date > date_paques + datetime.timedelta(days=35) and new_day_date < date_paques + datetime.timedelta(days=39):
        if new_day_date == date_paques + datetime.timedelta(days=36):
        ...
        elif new_day_date == date_paques + datetime.timedelta(days=37):
            comment_rogations = "\n\\item In Officio et in ML nihil fit de Rogationibus, præter antiphonas ad \\textit{Benedictus} et \\textit{Magnificat}." if "de ea" in dict_new_day["header"] else (
                "\n\\item In Officio et in ML nihil fit de Rogationibus, præter antiphonam ad \\textit{Magnificat}." if "memoria minor" in dict_new_day["header"] else "")
        elif new_day_date == date_paques + datetime.timedelta(days=38):
            comment_rogations = "\n\\item In Officio et in ML nihil fit de Rogationibus, præter antiphonam ad \\textit{Benedictus}." if "de ea" in dict_new_day[
                "header"] else ""
