...
def dict_sancto_create(current_year, even_year, year_letter, dict_tempo, paques, nb_dim_ap_pentec):
    ...
    def is_septuagesime(date):
        if date > paques - datetime.timedelta(days = 63): return True
        else: return False
    
    def is_careme(date):
        if date > paques - datetime.timedelta(days = 46) and date < paques: return True
        else: return False
    ...
    imm_conc["II_vesp"] = "\n\\item Vesperæ sollemnitatis ; benedictio Sanctissimi Sacramenti." if imm_conc_date.weekday() != 5 else "\n\\item I Vesperæ Dominicæ."
    ...
    vigiles_quatre_temps = "lectiones SO in supplemento 5; " if saint_jean_de_la_croix_date.weekday() == 2 else "" # Car si mercredi, forcément Quatre-Temps.
    saint_jean_de_la_croix["header"] = " - \\textsc{S. Ioannis a Cruce}, presbyteri et Ecclesiæ doctoris - \\textbf{memoria maior} - \\textit{Alb.} (olim die 24 novembris)."
    saint_jean_de_la_croix["body"] = "\n\\item ad Vigilias: " + vigiles_quatre_temps + "lectio de memoria in supplemento 61.\n\\item ad Benedictus: ø \\textit{Qui vult} (AM 644).\n\\item \\textit{in ML: Missa in PAL.}\n\\item in MC: præfatio de sanctis virginibus et religiosis."
    ...
    if sainte_marie_mere_de_dieu_date.weekday() == 4:
        dict_tempo[sainte_marie_mere_de_dieu_date]["symbols"] = dict_tempo[sainte_marie_mere_de_dieu_date]["symbols"].replace(" µ", " ł")
        dict_tempo[sainte_marie_mere_de_dieu_date + datetime.timedelta(days = 1)]["symbols"] = " µ" + dict_tempo[sainte_marie_mere_de_dieu_date + datetime.timedelta(days = 1)]["symbols"]
    ...
    if feast_day != 6:
        ...
        for i in range(6 - feast_day):
            if i < 4: lect_nativ += "\n\\item[" + str(2 + i) + " ian.] " + current_lectiones[2 + i]
    ...
    epiphanie["header"] = " - ¬ \\textbf{\\MakeUppercase{In Epiphania Domini}} - \\textbf{sollemnitas maior} - \\textit{Alb.}" if epiphanie_date.weekday() != 6 else " \\textbf{\\textsc{Dominica II post Nativitatem}} - ¬  \\textbf{\\MakeUppercase{In Epiphania Domini}} - \\textbf{sollemnitas maior} - \\textit{Alb.}"
    ...
    if epiphanie_date.weekday() == 4:
        dict_tempo[epiphanie_date]["symbols"] = dict_tempo[epiphanie_date]["symbols"].replace(" µ", " ł")
        dict_tempo[epiphanie_date + datetime.timedelta(days = 1)]["symbols"] = " µ" + dict_tempo[epiphanie_date + datetime.timedelta(days = 1)]["symbols"]
    ...
    saint_hilaire["header"] = " - \\textsc{S. Hilarii}, episcopi et Ecclesiæ doctoris - \\textbf{memoria maior} - \\textit{Alb.} (olim die 14 huius)."
    ...
    ss_maur_placide["header"] = " - \\textsc{Ss. Mauri et Placidi}, discipulorum SPN Benedicti - \\textbf{memoria maior} - \\textit{Alb.} (olim die 5 octobris)."
    ...
    saint_antoine["anniv"] = "Cras recurrit anniversarium erectionis S. Ioseph Claræ Vallis in titulum abbatialem (1992)."
    saint_antoine["header"] = " - \\textsc{S. Antonii}, abbatis - \\textbf{memoria maior} - \\textit{Alb.}"
    saint_antoine["body"] = "\n\\item in MC: præfatio de sanctis virginibus et religiosis."
    saint_antoine["II_vesp"] = "\n\\item ad Magnificat: ø \\textit{Vos qui reliquistis} (AM 624)."
    ...
    sainte_agnes["body"] = "\n\\item ad Vigilias pro breviario vetere: lectio de memoria in supplemento 71.\n\\item ad Laudes, Vesperas et Horas minores: antiphonæ propriæ.\n\\item in MC: Commune virginis martyris (MR 924) ; lectiones propriæ: Apoc \\textbf{7}, 9-17 / Mt \\textbf{10}, 34-39 ; præfatio de sanctis martyribus."
    ...
    saint_francois_de_sales["header"] = " - \\textsc{S. Francisci de Sales}, episcopi et Ecclesiæ doctoris - \\textbf{memoria maior} - \\textit{Alb.} (olim die 29 huius)."
    saint_francois_de_sales["body"] = "\n\\item ad Vigilias: lectio de memoria in supplemento 73.\n\\item ad Benedictus: ø \\textit{Sapientiam} (AM 981).\n\\item \\textit{in ML: Missa in PAL.}\n\\item in MC: præfatio de sanctis pastoribus."
    ...
    sainte_angele_merici["header"] = " - S. Angelæ Merici, virginis - \\textit{memoria minor} - \\textit{Vir.}"
    ...
    saint_thomas_aquin["anniv"] = "Cras recurrit anniversarium approbationis Constitutionum Sancti Ioseph Claræ Vallis ab episcopo Divionensi (1988)."
    saint_thomas_aquin["header"] = " - \\textsc{S. Thomæ de Aquino}, presbyteri et Ecclesiæ doctoris - \\textbf{memoria maior} - \\textit{Alb.} (olim die 7 martii)."
    alleluia_ml = "\\item \\textit{in ML: \\emph{ß} Alleluia ut in Communi doctorum.}" if not is_septuagesime(saint_thomas_aquin_date) else ""
    saint_thomas_aquin["body"] = "\n\\item ad Benedictus: ø \\textit{Sapientia clamitat} (AM 581)." + alleluia_ml + "\\item in MC: Commune doctorum Ecclesiæ (MR 943) ; præfatio I de sanctis."
    ...
    if presentation_date.weekday() == 6: # Si dimanche, il faut calculer quel dimanche c'est dans la forme extraordinaire (dans la forme ordinaire, c'est forcément le IVe dim. Per annum).
        pres_dim = " \\textbf{\\textsc{Dominica IV per annum}}" 
        if presentation_date.day == paques - datetime.timedelta(days = 63): pres_dim += " (Septuagesima)" 
        elif presentation_date.day == paques - datetime.timedelta(days = 56): pres_dim += " (Sexagesima)" 
        elif presentation_date.day == paques - datetime.timedelta(days = 49): pres_dim += " (Quinquagesima)" 
        else: pres_dim += " (IV post Epiphaniam)"
    else: pres_dim = ""
    presentation["header"] = pres_dim + " - \\textbf{\\textsc{In Præsentatione Domini}} (vel Purificationis Beatæ Mariæ Virginis) - \\textbf{festum} - \\textit{Alb.}"
    ...
    if presentation_date.weekday() == 4:
        ...
        dict_tempo[presentation_date + datetime.timedelta(days = 1)]["symbols"] = " µ" + dict_tempo[presentation_date + datetime.timedelta(days = 1)]["symbols"]
    ...
    if not is_careme(saint_jerome_emilien_date):
        ...
        saint_jerome_emilien["header"] = " - S. Hieronymi Emiliani - \\textit{memoria minor} - \\textit{Vir.}"
    ...
    sainte_scholastique["anniv"] = "Cras recurrit anniversarium publicæ renovationis votorum nostrorum (1988)."
    ...
    if not is_careme(sept_saints_fondateurs_date):
        ...
        sept_saints_fondateurs["header"] = " - Ss. Septem Fundatorum Ordinis Servorum Beatæ Mariæ Virginis - \\textit{memoria minor} - \\textit{Vir.} (olim die 12 februarii)."
        sept_saints_fondateurs["body"] = "\n\\item Ad Benedictus: ø \\textit{Ecce quam} in tono VIII g (AM 821), ß \\textit{Hi viri}; oratio in supplemento 103.\n\\item \\textit{in ML: Alb.}\n\\item in MC (Alb.): Commune sanctorum et sanctarum (MR 961)."
    ...
    if is_careme(saint_pierre_damien_date):
        ...
        saint_pierre_damien["header"] = " - \\textsc{S. Petri Damiani}, episcopi et Ecclesiæ doctoris - \\textbf{memoria maior} - \\textit{Alb.} (olim die 23 huius)."
    ...
    if is_careme(saint_polycarpe_date):
        ...
        saint_polycarpe["header"] = " - \\textsc{S. Polycarpi}, episcopi et martyris - \\textbf{memoria maior} - \\textit{Rub.} (olim die 26 ianuarii)."
    ...
    anniv_premier_mars["anniv"] = "Cras recurrit anniversarium fundationis Monasterii Dominæ Nostræ Mayliliensis (1946) ; anniversarium benedictionis Ecclesiæ Sanctissimi Cordis Iesu (1979) ; \\textup{†} anniversarium obitus RP Ludovici Mariæ \\textsc{Barrielle}, benefactoris (1983) ; \\textup{†} anniversarium obitus Eminentissimi Cardinalis Ioannis \\textsc{Balland}, benefactoris (1998)."
    ...
    if is_careme(saintes_perpetue_felicitee_date):
        ...
        saintes_perpetue_felicitee["body"] = "\n\\item ad Laudes: pro commemoratione Ss. Perpetuæ et Felicitatis, ø \\textit{Istarum} (AM 748) ; oratio vide ad diem 6 martii.\n\\item in MC: collecta de sanctis." 
    else:
        saintes_perpetue_felicitee["force"] = 40
        saintes_perpetue_felicitee["header"] = " - \\textsc{Ss. Perpetuæ et Felicitatis}, martyrum - \\textbf{memoria maior} - \\textit{Rub.} (olim die 6 martii)."
        saintes_perpetue_felicitee["body"] = "\n\\item ad Vigilias pro breviario vetere: lectio in supplemento 111.\n\\item ad Benedictus et Magnificat: ø \\textit{Istarum} (AM 691).\n\\item in MC: Commune martyrum (MR 909) ; præfatio de sanctis martyribus."
    ...
    saint_joseph["body"] = "\n\\item in MC: lectiones propriæ: 2 Sam \\textbf{7}, 4-5a.12-14a.16 / Rom \\textbf{4}, 13.16-18.22 / " + evg_mc + " ; præfatio propria (\\textit{Et te in sollemnitate})."
    ...
    if saint_joseph_date.weekday() == 4 and saint_joseph_date < paques:
        dict_tempo[saint_joseph_date]["symbols"] = dict_tempo[saint_joseph_date]["symbols"].replace(" µ", " ł")
        dict_tempo[saint_joseph_date + datetime.timedelta(days = 1)]["symbols"] = " µ" + dict_tempo[saint_joseph_date + datetime.timedelta(days = 1)]["symbols"]
    ...
    inv_sem_vigiles = "\n\\item ad Vigilias: antiphonæ et psalmi hebdomadæ " + hebdo_psalterii_inv[((saint_benoit_transitus_date - datetime.date(2011, 11, 27)).days // 7) % 2] + "." if saint_joseph_date.weekday() <= 4 else "" 
    ...
    saint_benoit_transitus["II_vesp"] = "\n\\item ad Vesperas: ¶ breve, tono simplici (AM 961)."
    # Si vendredi de carême, reporter le jeûne au lendemain:
    if saint_benoit_transitus_date.weekday() == 4 and saint_benoit_transitus_date < paques:
        dict_tempo[saint_benoit_transitus_date]["symbols"] = dict_tempo[saint_benoit_transitus_date]["symbols"].replace(" µ", " ł")
        dict_tempo[saint_benoit_transitus_date + datetime.timedelta(days = 1)]["symbols"] = " µ" + dict_tempo[saint_benoit_transitus_date + datetime.timedelta(days = 1)]["symbols"]
    ...
    if annonciation_date.weekday() == 4 and annonciation_date < paques:
        ...
        dict_tempo[annonciation_date + datetime.timedelta(days = 1)]["symbols"] = " µ" + dict_tempo[annonciation_date + datetime.timedelta(days = 1)]["symbols"]
    ...
    if not is_careme(saint_stanislas_ep_date):
        ...
        saint_stanislas_ep["header"] = " - S. Stanislai, episcopi et martyris - \\textit{memoria minor} - \\textit{Alb.} (olim die 7 maii)."
    ...
    saint_louis_marie["header"] = " - S. Ludovici Mariæ Grignion de Montfort, presbyteri - \\textit{memoria minor} - \\textit{Alb.} "
    ...
    sainte_catherine_sienne["header"] = " - \\textbf{\\textsc{S. Catharinæ Senensis, Europæ Patronæ, virginis et Ecclesiæ doctoris}} - \\textbf{festum} - \\textit{Alb.} (olim die 30 huius)."
    sainte_catherine_sienne["body"] = "\n\\item in Officio: omnia de Communi virginis, præter sequentia.\n\\item oratio in supplemento 118*.\n\\item ad Vigilias: lectiones II nocturni in supplemento 117*.\n\\item ad Benedictus: ø \\textit{O beata anima} cum \\textit{alleluia} (AM 1131).\n\\item \\textit{in ML: præfatio de sanctis.}\n\\item in MC: lectiones propriæ: 1 Io \\textbf{1}, 5 – \\textbf{2}, 2 / Mt \\textbf{25}, 1-13 ; præfatio de sanctis virginibus et religiosis."
    sainte_catherine_sienne["II_vesp"] = "\n\\item ad Magnificat: ø \\textit{O dignissima} cum \\textit{alleluia} (AM 1133)."
    
    saint_pie_V_date = datetime.date(current_year, 4, 30)
    saint_pie_V = dict_sancto[saint_pie_V_date] = {}
    saint_pie_V["force"] = 20
    saint_pie_V["header"] = " - S. Pii V, Papæ - \\textit{memoria minor} - \\textit{Alb.} (olim die 5 maii)."
    ...
    sts_philippe_jacques["header"] = " - \\textbf{\\textsc{Ss. Philippi et Iacobi, apostolorum}} - \\textbf{festum} - \\textit{Rub.} (olim die 1 maii [AM] et 11 maii [ML])."
    sts_philippe_jacques["body"] = "\n\\item in MC: lectiones propriæ: 1 Cor \\textbf{15}, 1-8 / Io \\textbf{14}, 6-14 ; præfatio I de Apostolis."
    sts_philippe_jacques["II_vesp"] = "\n\\item ad Magnificat: ø \\textit{Non turbetur} (AM 892)." if not even_year else ""
    
    abbes_cluny_date = datetime.date(current_year, 5, 11)
    abbes_cluny = dict_sancto[abbes_cluny_date] = {}
    abbes_cluny["force"] = 40
    abbes_cluny["header"] = " - \\textsc{Ss. Odonis, Maioli, Odilonis, Hugonis et B. Petri Venerabilis}, abbatum Cluniacensium - \\textbf{memoria maior} - \\textit{Alb.} (olim die 29 aprilis)."
    ...
    nd_fatima["body"] = "\n\\item ad Benedictus: ø \\textit{Ait Dominus} (AM 765) ; oratio in supplemento 118*." + messe_lue + messe_conv
    
    saint_matthias_date = datetime.date(current_year, 5, 14)
    saint_matthias = dict_sancto[saint_matthias_date] = {}
    saint_matthias["force"] = 70
    saint_matthias["header"] = " - \\textbf{\\textsc{S. Matthiæ, Apostoli}} - \\textbf{festum} - \\textit{Rub.} (olim die 24 februarii)."
    if saint_matthias_date < paques + datetime.timedelta(days = 49):
        ...
        messe_lue = "\\item \\textit{in ML: Missa tempore paschali (antiphona ad introitum, versus alleluiatici, antiphonæ ad offertorium et ad communionem sumuntur ex missa \\emph{Protexisti}).}"
    else:
        office = ""
        messe_lue = ""
    saint_matthias["body"] = office + "\\item ad Benedictus: ø \\textit{Sancti et iusti} (AM 637a)." + messe_lue + "\\item in MC: lectiones propriæ: Act \\textbf{1}, 15-17.20a.20c-26 / Io \\textbf{15}, 9-17 ; præfatio II de Apostolis."
    ...
    saint_jean_Ier["body"] = ant_bened + "\\item \\textit{in ML (Rub.): Missa de Communi summorum pontificum.}\n\\item in MC \\textit{(Rub.)}: Commune pastorum (MR 927)."
    ...
    saint_pierre_celestin["header"] = " - S. Petri Celestini, papæ et eremitæ - \\textit{memoria minor} - " + couleur
    saint_pierre_celestin["body"] = "\n\\item ad Benedictus: ø \\textit{Sacerdos et pontifex} (AM 656)." + messe_lue + messe_conv
    
    anniv_pere_bernard_date = datetime.date(current_year, 5, 21)
    anniv_pere_bernard = dict_sancto[anniv_pere_bernard_date] = {}
    anniv_pere_bernard["anniv"] = "\\textup{†} Cras recurrit anniversarium obitus RP Bernardi Mariæ \\textsc{Dewilde}, religiosis, qui die 21 maii 2014 obdormivit in Domino."
    
    saint_bede_date = datetime.date(current_year, 5, 25)
    saint_bede = dict_sancto[saint_bede_date] = {}
    saint_bede["force"] = 40
    saint_bede["header"] = " - \\textsc{S. Bedæ venerabilis}, presbyteri et Ecclesiæ doctoris - \\textbf{memoria maior} - \\textit{Alb.} (olim die 27 huius)."
    alleluia_bened = " cum \\textit{alleluia}" if saint_bede_date < paques + datetime.timedelta(days = 49) else ""
    saint_bede["body"] = "\n\\item ad Benedictus: ø \\textit{Qui verbum}" + alleluia_bened + " (AM 325).\n\\item in MC: Commune doctorum Ecclesiæ (MR 943) ; præfatio de sanctis virginibus et religiosis."
    ...
    saint_gregoire_VII["header"] = " - S. Gregorii VII, papæ - \\textit{memoria minor} - \\textit{" + couleur + "} (vide die 25 huius mensis)."
    saint_gregoire_VII["body"] = "\n\\item ad Benedictus: ø \\textit{Dum esset} (AM 663)." + messe_lue + messe_conv
    ...
    saint_augustin_cantorbery["header"] = " - S. Augustini Cantuariensis, episcopi - \\textit{memoria minor} - \\textit{" + couleur + "} (olim die 26 huius, vel 28 in ML)."
    saint_augustin_cantorbery["body"] = "\n\\item ad Benedictus: ø \\textit{Amavit eum} (AM 663)." + messe_lue + messe_conv
    ...
    visitation["force"] = 70
    visitation["header"] = " - \\textbf{\\textsc{In Visitatione Beatæ Mariæ Virginis}} - \\textbf{festum} - \\textit{Alb.} (olim die 2 iulii)."
    suppl_vigiles = "\\item ad Vigilias: in supplemento 122 ; invitatorium proprium in supplemento 58. " if visitation_date < paques + datetime.timedelta(days = 56) else ""
    lectures_mc = "Rom \\textbf{12}, 9-16b" if even_year else "Soph \\textbf{3}, 14-18a"
    visitation["body"] = suppl_vigiles + "\\item in MC: lectiones propriæ: " + lectures_mc + " / Lc \\textbf{1}, 39-56 ; præfatio II de Beata Maria Virgine."
    ...
    saint_ephrem["header"] = " - S. Ephræm, diaconi et Ecclesiæ doctoris - \\textit{memoria minor} - " + couleur + " (olim die 18 iunii)."
    saint_ephrem["body"] = "\n\\item ad Benedictus: ø \\textit{Similabo eum} (AM 669)." + messe_lue + "\\item in MC" + messe_conv + ": Commune doctorum Ecclesiæ (MR 944)."
    ...
    vigiles = "\\item ad Vigilias: invitatorium et hymnus de Communi Apostolorum extra tempus paschale ; lectio in supplemento 132 cum ¶ extra tempus paschale ; in II nocturno lectio brevis in supplemento 133 vel ut ad Sextam in Communi Apostolorum." if saint_barnabe_date < paques + datetime.timedelta(days = 56) else "\\item ad Vigilias: lectio in supplemento 132 pro breviario vetere ; in II nocturno lectio brevis in supplemento 133 vel ut ad Sextam in Communi Apostolorum."
    saint_barnabe["body"] = vigiles + "\\item \\textit{in ML: dicitur \\emph{Credo}.}\n\\item in MC: lectiones propriæ: Act \\textbf{11}, 21b-26 ; \\textbf{13}, 1-3 / Mt \\textbf{10}, 7-13 ; præfatio I de Apostolis."
    ant_magnif = " cum \\textit{alleluia}" if saint_barnabe_date < paques + datetime.timedelta(days = 49) else ""
    saint_barnabe["II_vesp"] = "\n\\item ad Magnificat: ø \\textit{Tradent enim vos}" + ant_magnif + " (AM 621)."
    ...
    messe_conv = "(MR 921)" if saint_antoine_padoue_date < paques + datetime.timedelta(days = 49) else "(MR 915)"
    saint_antoine_padoue["header"] = " - \\textsc{S. Antonii de Padova}, presbyteri et Ecclesiæ doctoris - \\textbf{memoria maior} - \\textit{Alb.}"
    in_officio = "\\item in Officio: oratio in supplemento 134." if saint_antoine_padoue_date < paques + datetime.timedelta(days = 56) else ""
    ...
    saint_romuald["header"] = " - \\textsc{S. Romualdi}, abbatis - \\textbf{memoria maior} - \\textit{Alb.} (olim die 7 februarii)."
    ...
    saint_louis_gonzague["header"] = " - S. Aloisii Gonzaga, religiosi - \\textit{memoria minor} - \\textit{Vir.}"
    saint_louis_gonzague["body"] = "\n\\item ad Benedictus: ø \\textit{Sanctorum velut} (AM 652).\n\\item \\textit{in ML: Alb.}\n\\item in MC: \\textit{Alb.}"
    ...
    saint_john_fisher["header"] = " - Ss. Ioannis Fisher, episcopi et Thomæ More, martyrum - \\textit{memoria minor} - \\textit{Vir.}"
    ...
    vigile_saint_jean_baptiste_date = datetime.date(current_year, 6, 23) if paques != datetime.date(current_year, 4, 25) and paques != datetime.date(current_year, 4, 17) else datetime.date(current_year, 6, 24)
    vigile_saint_jean_baptiste = dict_sancto[vigile_saint_jean_baptiste_date] = {}
    vigile_saint_jean_baptiste["force"] = 10
    vigile_saint_jean_baptiste["body"] = "\n\\item \\textit{in ML (Viol): Missa de Vigilia Nativitatis S. Ioannis Baptistæ.}"
    
    saint_jean_baptiste_date = datetime.date(current_year, 6, 24) if paques != datetime.date(current_year, 4, 25) and paques != datetime.date(current_year, 4, 17) else datetime.date(current_year, 6, 25) # On envisage le cas où Fête-Dieu ou Sacré-Cœur le 24. À ce moment-là, on décale S.J.B. au 25.
    ...
    if saint_jean_baptiste_date.weekday() == 6:
        ...
        if num_dim_ap_pentec == 2: text_ap_pentec = "II post Pentecosten vel infra octavam Ss. Corporis Christi"
        elif num_dim_ap_pentec == 3: text_ap_pentec = "III post Pentecosten vel infra octavam Ss. Cordis Iesu"
        else: text_ap_pentec = f_roman_numbers(num_dim_ap_pentec) + " post Pentecosten"
        text_dim = " \\textbf{\\textsc{Dominica " + num_dim_per_annum + " per annum}}" + " ("+ text_ap_pentec + ")"
    else: text_dim = ""
    saint_jean_baptiste["header"] = text_dim + " - þ \\textbf{\\MakeUppercase{In nativitate S. Ioannis Baptistæ}} - \\textbf{sollemnitas minor} - \\textit{Alb.}"
    hymne_laudes = "\n\\item ad Laudes : hymnus \\textit{Antra deserti} (AM 928a)." if saint_jean_baptiste_date.day == 25 or paques == datetime.date(current_year, 4, 24) or paques == datetime.date(current_year, 4, 16) else ""
    ...
    hymne_vepres = " ; hymnus \\textit{O nimis felix} (AM 926)" if saint_jean_baptiste_date.day == 25 or paques == datetime.date(current_year, 4, 24) or paques == datetime.date(current_year, 4, 16) else ""
    saint_jean_baptiste["II_vesp"] = "\n\\item Vesperæ sollemnitatis" + hymne_vepres + "."
    ...
    if paques != datetime.date(current_year, 4, 25) and paques != datetime.date(current_year, 4, 17): # Car si Fête-Dieu ou Sacré-Cœur le 24, alors S.J.B le 25, et il ne faut pas écraser la solennité avec l'anniversaire.
    ...
    saint_cyrille_alexandrie["header"] = " - S. Cyrilli Alexandrini, episcopi et Ecclesiæ doctoris - \\textit{memoria minor} - \\textit{Vir.} (olim die 9 februarii)."
    ...
    saint_cyrille_alexandrie["body"] = ant_bened + "\\item \\textit{in ML: Alb.}\n\\item in MC \\textit{(Alb.)}: Commune pastorum (MR 929)."
    ...
    saints_pierre_et_paul_date = datetime.date(current_year, 6, 29) if paques != datetime.date(current_year, 4, 22) else datetime.date(current_year, 6, 30) # On envisage le cas où Sacré-Cœur le 29. À ce moment-là, on décale S.P.P. au 30.
    saints_pierre_et_paul = dict_sancto[saints_pierre_et_paul_date] = {}
    saints_pierre_et_paul["force"] = 110
    saints_pierre_et_paul["I_vesp"] = "\\item I Vesperæ sollemnitatis sequentis ; ad Magnificat: ø \\textit{Gloriosi} (AM 958)."
    ...
    saints_pierre_et_paul["header"] = text_dim + " - þ \\textbf{\\MakeUppercase{Ss. Petri et Pauli, Apostolorum}} - \\textbf{sollemnitas minor} - \\textit{Rub.}"
    ...
    saints_pierre_et_paul["II_vesp"] = "\n\\item Vesperæ sollemnitatis ut in AM 939, præter antiphonas et capitulum de commemoratione S. Pauli Apostoli (AM 941), cum psalmis ut in II Vesperis de Communi Apostolorum."
    
    if paques != datetime.date(current_year, 4, 22): # Car si Sacré-Cœur le 29, alors S.P.P. le 30, et il ne faut pas écraser la solennité avec les Protomartyrs.
        ...
        saints_protomartyrs["header"] = " - Ss. Protomartyrum S. Romanæ Ecclesiæ - \\textit{memoria minor} - \\textit{Vir.}"
    ...
    saint_thomas_ap["header"] = " - \\textbf{\\textsc{S. Thomæ, apostoli}} - \\textbf{festum} - \\textit{Rub.} (olim die 21 decembris)."
    # Si 29/06 et 03/07 appartiennent à la même semaine, on inverse la semaine du psautier pour les Vigiles:
    inv_sem_vigiles = "; antiphonæ et psalmi hebdomadæ " + hebdo_psalterii_inv[((saints_pierre_et_paul_date - datetime.date(2011, 11, 27)).days // 7) % 2] if saints_pierre_et_paul_date.weekday() == 6 or saints_pierre_et_paul_date.weekday() < saint_thomas_ap_date.weekday() else ""
    ...
    saint_thomas_ap["II_vesp"] = "\n\\item ad Magnificat: ø \\textit{Misi digitum} (AM 480)."
    # Si 1er vendredi du mois, reporter le jeûne au lendemain:
    if saint_thomas_ap_date.weekday() == 4:
        dict_tempo[saint_thomas_ap_date]["symbols"] = dict_tempo[saint_thomas_ap_date]["symbols"].replace(" µ", " ł")
        dict_tempo[saint_thomas_ap_date + datetime.timedelta(days = 1)]["symbols"] = " µ" + dict_tempo[saint_thomas_ap_date + datetime.timedelta(days = 1)]["symbols"]
    ...
    sainte_maria_goretti["header"] = " - S. Mariæ Goretti, virginis et martyris - \\textit{memoria minor} - \\textit{Vir.}"
    ...
    if saint_benoit_date.weekday() == 6:
        ...
        num_dim_per_annum = f_roman_numbers(num_dim_ap_pentec + 35 - nb_dim_ap_pentec)
        text_ap_pentec = f_roman_numbers(num_dim_ap_pentec) + " post Pentecosten"
        text_dim = " \\textbf{\\textsc{Dominica " + num_dim_per_annum + " per annum}}" + " ("+ text_ap_pentec + ")"
    else: text_dim = ""
    saint_benoit["header"] = text_dim + " - ¬ \\textbf{\\MakeUppercase{S. P. N. Benedicti, Abbatis,}} \\textsc{Europæ patroni} - \\textbf{sollemnitas maior} - \\textit{Alb.}"
    ...
    saint_jean_gualbert["header"] = " - S. Ioannis Gualberti, abbatis - \\textit{memoria minor} - \\textit{Vir.}"
    ...
    saint_camille["header"] = " - S. Camilli de Lellis, presbyteri - \\textit{memoria minor} - \\textit{Vir.} (olim die 18 iulii)."
    ...
    saint_bonaventure["header"] = " - \\textsc{S. Bonaventuræ}, episcopi et Ecclesiæ doctoris - \\textbf{memoria maior} - \\textit{Alb.} (olim die 14 huius)."
    ...
    saint_laurent_brindisi["header"] = " - S. Laurentii de Brindisi, presbyteri et Ecclesiæ doctoris - \\textit{memoria minor} - \\textit{Vir.} "
    ...
    lectures_mc = "1 Tim \\textbf{5}, 3-10 / Lc \\textbf{2}, 36-38" if even_year else "Iudt \\textbf{8}, 2-8 / Mt \\textbf{5}, 13-16"
    ...
    saint_pierre_chrysologue["header"] = " - S. Petri Chrysologi, episcopi et Ecclesiæ doctoris - \\textit{memoria minor} - \\textit{Vir.} (olim die 4 decembris)."
    ...
    saint_alphonse_liguori["header"] = " - \\textsc{S. Alfonsi de Liguori}, episcopi et Ecclesiæ doctoris - \\textbf{memoria maior} - \\textit{Alb.} (olim die 2 huius)."
    saint_alphonse_liguori["body"] = "\n\\item ad Vigilias: lectio in supplemento 149.\n\\item ad Benedictus: ø \\textit{Observa} (AM 582).\n\\item in MC: præfatio de sanctis pastoribus."
    ...
    dedicace_nd_neiges["header"] = " - Dedicatio basilicæ S. Mariæ - \\textit{memoria minor} - \\textit{Vir.}"
    ...
    if transfiguration_date.weekday() == 6:
        lectures_mc = "Dan \\textbf{7}, 9-10.13-14 / 2 Petr \\textbf{1}, 16-19" + " / " + evang_mc[year_letter]
    elif even_year:
        lectures_mc = "2 Petr \\textbf{1}, 16-19" + " / " + evang_mc[year_letter]
    else:
    ...
    if assomption_date.weekday() == 6:
        ...
        num_dim_per_annum = f_roman_numbers(num_dim_ap_pentec + 35 - nb_dim_ap_pentec)
        num_summer = " - " + f_num_summer(assomption_date)[0] + f_num_summer(assomption_date)[1]
        text_ap_pentec = f_roman_numbers(num_dim_ap_pentec) + " post Pentecosten" + num_summer
        text_dim = " \\textbf{\\textsc{Dominica " + num_dim_per_annum + " per annum}}" + " ("+ text_ap_pentec + ")"
    else: text_dim = ""
    assomption["header"] = text_dim + " - ¬ \\textbf{\\MakeUppercase{In assumptione Beatæ Mariæ Virginis}}, \\textsc{Patronæ Principalis Galliæ} - \\textbf{sollemnitas maior} - \\textit{Alb.}"
    ...
    saint_pie_X["header"] = " - \\textsc{S. Pii X}, papæ - \\textbf{memoria maior} - \\textit{Alb.} (olim die 3 septembris)."
    ...
    sainte_monique["header"] = " - \\textsc{S. Monicæ, viduæ} - \\textbf{memoria maior} - \\textit{Alb.} (olim die 4 maii)."
    ...
    saint_gregoire["header"] = " - \\textbf{S. Gregorii I, papæ et Ecclesiæ doctoris} - \\textbf{festum} - \\textit{Alb.} (olim die 12 martii)."
    ...
    sainte_reine["I_vesp"] = "\\item I Vesperæ sollemnitatis sequentis. Responsorium \\textit{Adiuvabit eam} et versiculum in AM 1178."
    if sainte_reine_date.weekday() == 6: # Si tombe un dimanche, on calcule quel dimanche (per annum, post Pentecosten).
        num_dim_ap_pentec = (sainte_reine_date - pentecote).days // 7
        num_dim_per_annum = f_roman_numbers(num_dim_ap_pentec + 35 - nb_dim_ap_pentec)
        num_summer = " - " + f_num_summer(sainte_reine_date)[0] + f_num_summer(sainte_reine_date)[1]
        text_ap_pentec = f_roman_numbers(num_dim_ap_pentec) + " post Pentecosten" + num_summer
        text_dim = " \\textbf{\\textsc{Dominica " + num_dim_per_annum + " per annum}}" + " ("+ text_ap_pentec + ")"
    else: text_dim = ""
    sainte_reine["header"] = text_dim + " - þ \\textbf{\\MakeUppercase{S. Reginæ, virginis et martyris}} - \\textbf{sollemnitas minor} - \\textit{Rub.}"
    ...
    sainte_reine["II_vesp"] = "\n\\item Vesperæ sollemnitatis. Responsorium \\textit{Adiuvabit eam} et versiculum in AM 1178." if sainte_reine_date.weekday() == 5 else "\\item ad Vesperas, responsorium \\textit{Adiuvabit eam} et versiculum in AM 1178."
    # Si 1er vendredi du mois, reporter le jeûne au lendemain:
    if sainte_reine_date.weekday() == 4:
        dict_tempo[sainte_reine_date]["symbols"] = dict_tempo[sainte_reine_date]["symbols"].replace(" µ", " ł")
    ...
    hebdo_psalt_vigiles = hebdo_psalterii_inv[((sainte_reine_date - datetime.date(2011, 11, 27)).days // 7) % 2]
    ...
    nativite_bmv["body"] = "\n\\item ad Vigilias: antiphonæ et psalmi hebdomadæ " + hebdo_psalt_vigiles + " ; pro breviario vetere: in II nocturno vide in supplemento 169." + ant_bened + "\\item in MC: lectiones propriæ: " + lectures_mc + " / Mt \\textbf{1}, 18-23 \\textit{(formula brevior)} ; præfatio I de Beata Maria Virgine."
    ...
    saint_jean_chrysostome["header"] = " - \\textsc{S. Ioannis Chrysostomi}, episcopi et Ecclesiæ doctoris - \\textbf{memoria maior} - \\textit{Alb.} (olim die 27 ianuarii)."
    saint_jean_chrysostome["body"] = "\n\\item ad Vigilias: lectio, ¶ et oratio in supplemento 169.\n\\item ad Benedictus: ø \\textit{Semen est} (AM 324).\n\\item in MC: præfatio de sanctis pastoribus."
    
    sainte_croix_date = datetime.date(current_year, 9, 14)
    sainte_croix = dict_sancto[sainte_croix_date] = {}
    sainte_croix["force"] = 90
    if sainte_croix_date.weekday() == 6: # Si tombe un dimanche, on calcule quel dimanche (per annum, post Pentecosten) :
        num_dim_ap_pentec = (sainte_croix_date - pentecote).days // 7
        num_dim_per_annum = f_roman_numbers(num_dim_ap_pentec + 35 - nb_dim_ap_pentec)
        num_summer = " - " + f_num_summer(sainte_croix_date)[0] + f_num_summer(sainte_croix_date)[1]
        text_ap_pentec = f_roman_numbers(num_dim_ap_pentec) + " post Pentecosten" + num_summer
        text_dim = " \\textbf{\\textsc{Dominica " + num_dim_per_annum + " per annum}}" + " ("+ text_ap_pentec + ")"
    else: text_dim = ""
    sainte_croix["header"] = text_dim + " - \\textbf{\\textsc{In Exaltatione Sanctæ Crucis}} - \\textbf{festum} - \\textit{Rub.}"
    lectures_mc = "Nb \\textbf{21}, 4b-9 / " if sainte_croix_date.weekday() == 6 else ""
    vepres = "\\item ad Vesperas: omnia ut in I Vesperis præter ad Magnificat: ø \\textit{O Crux benedicta} (AM 1046)." if even_year or sainte_croix_date.weekday() == 6 else "\\item ad Vesperas: omnia ut in I Vesperis."
    sainte_croix["body"] = "\n\\item in MC: lectiones propriæ: " + lectures_mc + "Phil \\textbf{2}, 6-11 / Io \\textbf{3}, 13-17 ; præfatio propria." + vepres
    ...
    ant_bened = "" if even_year else "\\item ad Benedictus: ø \\textit{Tuam ipsius} in tono VI f (AM 867)."
    ...
    sept_douleurs["body"] = "\n\\item ad Vigilias pro breviario vetere: in supplemento 170.\n\\item ad Laudes, Vesperas et Horas minores: antiphonæ propriæ." + ant_bened + "\\item in MC: lectiones propriæ: Hebr \\textbf{5}, 7-9 / " + evg_mc + " ; sequentia ; præfatio I de Beata Maria Virgine." + ant_magnif
    ...
    if even_year:
        ...
        saint_robert_bellarmin["header"] = " - S. Roberti Bellarmino, episcopi et Ecclesiæ doctoris - \\textit{memoria minor} - \\textit{Vir.} (olim die 13 maii)."
    ...
    saints_archanges["body"] = "\n\\item omnia ut hucusque in festo S. Michaelis." + ant_bened + "\\item in MC: lectiones propriæ: Apoc \\textbf{12}, 7-12a / Io \\textbf{1}, 47-51 ; præfatio de Angelis."
    ...
    saints_anges_gardiens["body"] = "\n\\item ad Vigilias pro breviario vetere: in supplemento 175.\n\\item ad Laudes, Vesperas et Horas minores: antiphonæ propriæ." + ant_bened + "\\item in MC: cantatur hymnus angelicus \\textit{Gloria} ; lectiones propriæ: Ex \\textbf{23}, 20-23a / Mt \\textbf{18}, 1-5.10 ; præfatio de Angelis."
    ...
    saint_denis["header"] = " - Ss. Dionysii, episcopi et Sociorum, martyrum. - \\textit{memoria minor} - \\textit{Vir.}"
    ...
    sainte_marguerite_marie["header"] = " - S. Margaritæ Mariæ Alacoque, virginis - \\textit{memoria minor} - \\textit{Vir.} (olim die 17 huius)."
    ...
    saint_ignace_antioche["header"] = " - \\textsc{S. Ignatii Antiocheni}, episcopi et martyris - \\textbf{memoria maior} - \\textit{Rub.} (olim die 1 februarii)."
    ...
    saint_jean_brebeuf["header"] = " - Ss. Ioannis de Brebeuf et Isaac Jogues, presbyterorum et Sociorum, martyrum - \\textit{memoria minor} - \\textit{Vir.}"
    ...
    saint_jean_paul_II["header"] = " - S. Ioannis Pauli II, papæ - \\textit{memoria minor} - \\textit{Vir.}"
    ...
    saint_antoine_marie_claret["header"] = " - S. Antonii Mariæ Claret, episcopi - \\textit{memoria minor} - \\textit{Vir.}"
    ...
    if toussaint_date.weekday() == 4:
        dict_tempo[toussaint_date]["symbols"] = dict_tempo[toussaint_date]["symbols"].replace(" µ", " ł")
        dict_tempo[toussaint_date + datetime.timedelta(days = 1)]["symbols"] = " µ" + dict_tempo[toussaint_date + datetime.timedelta(days = 1)]["symbols"]
    
    comm_defunts_date = datetime.date(current_year, 11, 2)
    comm_defunts = dict_sancto[comm_defunts_date] = {}
    comm_defunts["force"] = 110
    if comm_defunts_date.weekday() == 6: # Si tombe un dimanche, on calcule quel dimanche (per annum, post Pentecosten).
        num_dim_ap_pentec = (comm_defunts_date - pentecote).days // 7
        num_dim_per_annum = f_roman_numbers(num_dim_ap_pentec + 35 - nb_dim_ap_pentec)
        num_summer = " - " + f_num_summer(comm_defunts_date)[0] + f_num_summer(comm_defunts_date)[1]
        text_ap_pentec = f_roman_numbers(num_dim_ap_pentec) + " post Pentecosten" + num_summer
        text_dim = " \\textbf{\\textsc{Dominica " + num_dim_per_annum + " per annum}}" + " ("+ text_ap_pentec + ")"
    else: text_dim = ""
    comm_defunts["header"] = text_dim + " - \\textbf{\\textsc{In Commemoratione omnium Fidelium Defunctorum}} - \\textit{Nigr.}."
    if comm_defunts_date.weekday() == 6:
        rajout_dimanche = "\n\\item hodie Missæ et Officium defunctorum, nihil fit de dominica."
    ...
    comm_defunts["body"] = rajout_dimanche + "\n\\item ß \\textit{Gloria Patri} dicitur in fine omnium psalmorum et canticorum. In responsoriis omittitur ß \\textit{Requiem æternam}.\n\\item ad Vigilias: absolute incipitur ab invitatorio (psalmus 94); " + nocturnes + "\n\\item \\textit{hodie, licet omnibus sacerdotibus tres Missas celebrare, ea tamen lege, ut unam tantum libere applicare et pro ea stipem percipere queant: tenentur vero, nulla stipe percepta, alteram in suffragium omnium fidelium defunctorum, tertiam ad mentem Summi Pontificis applicare.}\n\\item \\textit{ritus in Missis servandus: In prima et secunda Missa, si immediate sacerdos aliam Missam sit celebraturus, sumpto divino Sanguine, purificat calicem cum aqua tantum.}" + rajout_dim_ml + "\n\\item in MC \\textit{(1a Missa)}:" + rajout_dim_mc + " lectiones propriæ: Is \\textbf{25}, 6a-9 / 1 Cor \\textbf{15}, 51-54.57 ; sequentia \\textit{Dies iræ} (Besnier 53) / Io \\textbf{6}, 51-59 ;" + credo_mc + " præfatio I de defunctis."
    comm_defunts["II_vesp"] = ("\\item Vesperæ pro defunctis." if comm_defunts_date.weekday() == 5 else "") + "\\item Completorium pro defunctis: incipitur a \\textit{Confiteor}, post examen conscientiæ ; aspersio de more."
    # Si 1er vendredi du mois, reporter le jeûne au lendemain:
    if comm_defunts_date.weekday() == 4:
        dict_tempo[comm_defunts_date]["symbols"] = dict_tempo[comm_defunts_date]["symbols"].replace(" µ", " ł")
        dict_tempo[comm_defunts_date + datetime.timedelta(days = 1)]["symbols"] = " µ" + dict_tempo[comm_defunts_date + datetime.timedelta(days = 1)]["symbols"]
    ...
    saint_martin_porres["header"] = " - S. Martini de Porres, religiosi - \\textit{memoria minor} - \\textit{Vir.}"
    ...
    if dedicace_latran_date.weekday() == 6: # Si tombe un dimanche, on calcule quel dimanche (per annum, post Pentecosten).
        num_dim_ap_pentec = (dedicace_latran_date - pentecote).days // 7
        num_dim_per_annum = f_roman_numbers(num_dim_ap_pentec + 35 - nb_dim_ap_pentec)
        num_summer = " - " + f_num_summer(dedicace_latran_date)[0] + f_num_summer(dedicace_latran_date)[1]
        text_ap_pentec = f_roman_numbers(num_dim_ap_pentec) + " post Pentecosten" + num_summer
        text_dim = " \\textbf{\\textsc{Dominica " + num_dim_per_annum + " per annum}}" + " ("+ text_ap_pentec + ")"
    else: text_dim = ""
    ...
    if dedicace_latran_date.weekday() == 6:
        lectures_mc = "Ez \\textbf{47}, 1-2.8-9.12 / 1 Cor \\textbf{3}, 9b-11.16-17"
    elif even_year:
        lectures_mc = "1 Cor \\textbf{3}, 9b-11.16-17"
    else:
    ...
    saint_leon["force"] = 40
    saint_leon["header"] = " - \\textsc{S. Leonis magni}, papæ et Ecclesiæ doctoris - \\textbf{memoria maior} - \\textit{Alb.} (olim die 11 aprilis)."
    ...
    saint_martin["body"] = lectures_vigiles + "\\item in MC: lectiones propriæ: Is \\textbf{61}, 1-3a / Mt \\textbf{25}, 31-40 ; præfatio de sanctis pastoribus."
    ...
    saint_theodore_studite["header"] = " - S. Theodori Studitæ, abbatis - \\textit{memoria minor} - \\textit{Vir.}"
    ...
    inv_sem_vigiles = "; antiphonæ et psalmi hebdomadæ " + hebdo_psalterii_inv[((saint_martin_date - datetime.date(2011, 11, 27)).days // 7) % 2] if saint_martin_date.weekday() < 5 else ""
    saint_benigne["body"] = "\n\\item in Officio: omnia de Communi unius martyris ; oratio in supplemento 188.\n\\item ad Vigilias: invitatorium proprium in supplemento 59 ; omnia de Communi unius martyris præter hymnum in supplemento 185, lectiones I et II nocturnorum in supplemento 185*" + inv_sem_vigiles + ".\n\\item \\textit{in ML: Missa et præfatio propriæ (olim die 20 novembris).}\n\\item in MC: oratio propria ; Commune martyrum (MR 915) ; lectiones propriæ: 1 Thes \\textbf{2}, 2-8 / Mc \\textbf{16}, 15-18 ; præfatio de sanctis martyribus."
    ...
    saint_albert["header"] = " - S. Alberti Magni, episcopi et Ecclesiæ doctoris - \\textit{memoria minor} - \\textit{Vir.}"
    ...
    sainte_elisabeth_hongrie["header"] = " - S. Elisabeth Hungariæ - \\textit{memoria minor} - \\textit{Vir.}"
    ...
    dedicace_sts_pierre_paul["anniv"] = "Cras recurrit anniversarium ingressus et installationis nostræ in Flaviniacum (1976)."
    dedicace_sts_pierre_paul["header"] = " - Dedicatio Basilicarum Ss. Petri et Pauli, Apostolorum - \\textit{memoria minor }- \\textit{Vir.}"
    ...
    sainte_cecile["body"] = "\n\\item ad Vigilias pro breviario vetere: in supplemento 191.\n\\item ad Laudes, Vesperas et Horas minores: antiphonæ propriæ.\n\\item in MC: omnia in MP ; lectiones propriæ: Os \\textbf{2}, 16b.17b.21-22 /  \\textbf{25}, 1-13 ; præfatio de sanctis martyribus."
